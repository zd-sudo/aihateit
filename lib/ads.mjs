...
const ADSENSE_LOADER_RE =
  /[ \t]*<script async src="https:\/\/pagead2\.googlesyndication\.com\/pagead\/js\/adsbygoogle\.js\?client=ca-pub-\d{10,20}" crossorigin="anonymous"><\/script>\n?/g;

export function adsenseLoaderTag(publisherId) {
...
// Keep a single static loader in <head> so crawlers see the publisher id.
// Empty publisher id removes the tag. The manual slot still mounts at runtime.
export function applyAdsenseLoader(html, publisherId) {
  const source = String(html || "");
  const tag = adsenseLoaderTag(publisherId);
  const without = source.replace(ADSENSE_LOADER_RE, "");
  if (!tag) return without;
  const indented = `    ${tag}`;
  if (/<meta name="viewport"[^>]*>/.test(without)) {
    return without.replace(/(<meta name="viewport"[^>]*>)/, `$1\n${indented}`);
  }
  return without.replace(/<\/head>/i, `${indented}\n</head>`);
}
