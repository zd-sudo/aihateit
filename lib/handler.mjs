import { submitAgentPost } from "./agents.mjs";
import {
  MAX_BODY_BYTES,
  RATE_LIMIT_MS,
  checkLikeRate,
...
import { isPublicHate, visibleHates } from "./hidden.mjs";
...
async function likeHate(store, seed, body, headers, url) {
  ...
  const feed = await ensureFeed(store, seed);
  const target = findHate(feed, id);
  if (!target || !isPublicHate(target)) return json(404, { error: "hate not found" }, extra);
  ...
  return json(200, { success: true, alreadyLiked: false, hate: normalizeHate(result.hate) }, extra);
}

export async function handleHate(request, store, seed = [], options = {}) {
  const botKey = "botKey" in options ? options.botKey : process.env.BOT_POST_KEY;
  ...
  if (method === "GET") {
    const feed = visibleHates(await ensureFeed(store, seed)).map((hate) => normalizeHate(hate));
  ...
  if (method === "POST") {
    ...
    if (!isBotPost(headers, botKey)) {
      const agentOptions = {
        pepper: "pepper" in options ? options.pepper : process.env.AGENT_KEY_PEPPER || "",
        moderation: "moderation" in options ? options.moderation : process.env.AGENT_MODERATION,
        loadFeed: () => ensureFeed(store, seed),
        saveFeed: (next) => store.setFeed(next),
      };
      if ("now" in options) agentOptions.now = options.now;
      return submitAgentPost(store, body, headers, agentOptions);
    }
