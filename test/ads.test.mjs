...
test("env wins over the committed ads-config, missing id stays empty", () => {
  ...
  assert.deepEqual(fromFile, { publisherId: "", slotId: "" });
  assert.deepEqual(resolveAdsConfig({}, fromFile), { publisherId: "", slotId: "" });
  assert.deepEqual(
...
test("ads.txt is a placeholder until a publisher id exists, then one Google line", () => {
  assert.match(adsTxt, /publisher id/i);
  assert.match(adsTxt, /google\.com, pub-XXXXXXXXXXXXXXXX, DIRECT, f08c47fec0942fa0/);
  assert.doesNotMatch(adsTxt, /^google\.com,/m);
